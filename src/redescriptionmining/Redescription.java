...
public class Redescription implements Comparable<Redescription>{
    ...
    int disj(DataSetCreator dat, Mappings map){
        
        this.closeInterval(dat, map);
        this.createRuleString(map);
        
        HashSet<String> redAt = new HashSet<>();
        HashSet<String> redAt1 = new HashSet<>();
        
        String rrs = ruleStrings.get(0);
        String rrs1 = ruleStrings.get(1);
        String t[] = rrs.split(" OR ");
        for(int i=0;i<t.length;i++){
            t[i] = t[i].replaceAll("\\(1\\)", "");
            t[i] = t[i].replaceAll("\\(", "");
            t[i] = t[i].replaceAll("\\)", "");
            t[i] = t[i].replaceAll("NOT", "");
            String tt[] = t[i].split(" AND ");
            
            for(int j=0;j<tt.length;j++){
                tt[j]=tt[j].replaceAll("\\(1\\)", "");
                 tt[j]=tt[j].replaceAll("<=", "");
                if(tt[j].contains("<="))
                    tt[j] = tt[j].split(" <= ")[0].trim();
                 else if(tt[j].contains(">=")){
                    tt[j] = tt[j].split(" >= ")[0].trim();
                 }
                 else if(tt[j].contains(">"))
                    tt[j] = tt[j].split(" > ")[0].trim();
        ...
        for(int i=0;i<t.length;i++){
                t[i]=t[i].replaceAll("\\(1\\)", "");
                if(t[i].contains("<="))
                    t[i] = t[i].split(" <= ")[0].trim();
                else if(t[i].contains(">"))
                    t[i] = t[i].split(" > ")[0].trim();
                redAt1.add(t[i]);
            }           
        
        Iterator<String> it = redAt.iterator();
        while(it.hasNext()){
            if(redAt1.contains(it.next()))
                return 0;
        }
     ...
     int disj(Redescription R1, DataSetCreator dat, Mappings map){
        
         R1.closeInterval(dat, map);
         R1.createRuleString(map);
        ...
        String rrs = ruleStrings.get(0);
        String rrs1 = R1.ruleStrings.get(1);
        String t[] = rrs.split(" OR ");
        for(int i=0;i<t.length;i++){
             t[i] = t[i].replaceAll("\\(1\\)", "");
            t[i] = t[i].replaceAll("\\(", "");
        ...
        redAt.clear(); redAt1.clear();
        
        
         rrs = ruleStrings.get(1);
         rrs1 = R1.ruleStrings.get(0);
         t = rrs.split(" OR ");
        for(int i=0;i<t.length;i++){
             t[i] = t[i].replaceAll("\\(1\\)", "");
            t[i] = t[i].replaceAll("\\(", "");
   ...
   for(int sid=0;sid<viewElementsLists.size();sid++){
    ...
    TIntHashSet computeElements(ArrayList<Conjunction> side, DataSetCreator dat, Mappings map){
     ...
     ArrayList<TIntHashSet> computeElementsGen(DataSetCreator dat, Mappings map){
      ...
      ArrayList<TIntHashSet> computeElementsGen1(DataSetCreator dat, Mappings map, DataSetCreator dat1, Mappings map1){ //original - all
        ArrayList<TIntHashSet> elem=new ArrayList<>();//TIntHashSet(this.viewElementsLists.size()*dat.numExamples);
        ArrayList<ArrayList<TIntHashSet>> sideElemsNew = new ArrayList<>();
        //prvo dopuniti - promijeniti elemT ide-eveima nove mape
        
         for(int k=0;k<viewElementsLists.size();k++){ 
                ArrayList<Conjunction> side=viewElementsLists.get(k);
                sideElemsNew.add(new ArrayList<>());
                for(int i=0;i<side.size();i++){
                    sideElemsNew.get(k).add(new TIntHashSet());
                            for(int el=0;el<dat1.numExamples;el++){
                                TIntIterator ijit = side.get(i).attributes.iterator();
                                        int contained = 1;
                                     while(ijit.hasNext()){
                                         int at1 = ijit.next();
                                         String atS = map.idAtt.get(at1);
                                         int at = map1.attId.get(atS);
                                         
                                         if(!map.catAttInd.contains(at1)){
                                         
                                        /* System.out.println("Check R elements");
                                         System.out.println(side.get(i).Rule.contains(at1));
                                         System.out.println(side.get(i).Rule.get(at1).size());*/
                                         if(dat1.getValue(at, el)< side.get(i).Rule.get(at1).get(1) || dat1.getValue(at, el)>side.get(i).Rule.get(at1).get(3)){
                                             contained = 0;  
                                             break;
                                            }
                                         }
                                         else{
                                              if(map1.cattAtt.get(at).getValue0().keySet().contains(dat1.getValueCategorical(at, el))){
                                                  String cat=map1.cattAtt.get(at).getValue1().get((int)(double)side.get(i).Rule.get(at1).get(0));
                                                  String realCat=dat1.getValueCategorical(at, el);
                                                      if(!cat.contentEquals(realCat)){
                                                             contained = 0;
                                                     break;
                                                      }
                                     ...
                                     if(contained == 1){
                                             sideElemsNew.get(k).get(i).add(el);
                                         }
                              }
                }
          }
       ...
       for(int k=0;k<viewElementsLists.size();k++){ 
           ...
           ArrayList<Conjunction> side=viewElementsLists.get(k);
        for(int i=0;i<side.size();i++){
            TIntHashSet elemT=sideElemsNew.get(k).get(i);//side.get(i).elements;
          
            if(side.get(i).isNegated==false){//nije dobro, izracunati iz pravila
                TIntIterator it=elemT.iterator();
                ...
                for(int el=0;el<dat1.numExamples;el++){
                    if(!elemT.contains(el)){
                        ...
                        while(it.hasNext()){
                            ...
                            String atS = map.idAtt.get(at1);
                            int at = map1.attId.get(atS);
                            if(!map1.catAttInd.contains(at)){
                                if(dat1.getValue(at, el)!=Double.POSITIVE_INFINITY){
                                    tmpSet.add(el);
                                    break;
                                }
                            }
       ...
       // System.out.println("Target info....");
            while(it.hasNext()){
                ...
                if(!map.idExample.containsKey(el))
                    System.out.println("Not contained in mapping!");
                if(!targets.containsKey(elS))
                    System.out.println(elS+" "+"not among targets!");
                int tv = targets.get(elS);
                int tind = targetsIndex.get(tv);
                
              //  System.out.println(elS+" "+tind+" "+tv);
                
                int tvc = targetValueCounts.size();
                
                if((tind+1)>tvc){
                    for(int i=0;i<((tind+1)-tvc);i++)//zamijene se vrijednosti na nekim redsima, jedna 0 prijede u 1
