...
public class RedescriptionSet {
 ...
 public long NumTests = 0;
    
    public int createGuidedJoinExt(RuleReader rr, RuleReader rr1, Jacard js[], ApplicationSettings appset, int oldIndexRR, int oldIndexRR1, int RunInd, boolean outOfmemory[], Mappings map, DataSetCreator dat){
        ...
        for(int i=oldIndexRR;i<rr.rules.size();i++){
            ...
            for(int j=oldIndexRR1;j<rr1.rules.size();j++){
                ...
                if(appset.sameView == 1 && rr.rules.get(i).disjoint(rr1.rules.get(j))!=1)
                    continue;
         ...
         tmp.clearRuleMaps();
                 ...
                 for(int k=0;k<redescriptions.size();k++){
                    ...
                    if((maxMemory-(totalMemory-freemem)<minMemory)&& memoryCanbeOptimized==true){
                        ...
                        //add here for sameAt check in sameView case
                        
                        if(appset.sameView == 1)
                            if(tmp.disj(redescriptions.get(k),dat, map)==0)
                                continue;
            ...
            System.out.println("Computing disjunctive refinement...");
                         ...
                         if(appset.rightDisjunction==true){
                           ...
                           for(int j=oldIndexRR1;j<rr1.rules.size();j++){
                               ...
                               if(appset.sameView == 1)
                                    if(js[0].disj(redescriptions.get(k), rr1.rules.get(j), 1, dat, map)!=1)
                                        continue;
                         ...
                         if(appset.leftDisjunction==true){
                           ...
                           joinJS=0.0; maxJoinJS=0.0; maxInd=0; negated=0;
                                 for(int i=oldIndexRR;i<rr.rules.size();i++){
                                     ...
                                     if(appset.sameView == 1)
                                        if(js[0].disj(redescriptions.get(k), rr.rules.get(i), 0, dat, map)!=1)
                                                     continue;
        ...
        for(int i=0;i<numRedescriptions;i++){
           ...
           for(int z=0;z<redescriptions.get(i).viewElementsLists.size();z++){
               if(redescriptions.get(i).viewElementsLists.size()>0){
                   if(appset.sameView == 1)
                       redescriptions.get(i).ruleStrings.set(z,redescriptions.get(i).ruleStrings.get(z).replaceAll("\\(1\\)", ""));
           ...
           // System.out.println("JS: "+redescriptions.get(i).JS);
             ...
             bw.write("Target value counts: \n");
             for(int zz=0;zz<redescriptions.get(i).targetValueCounts.size();zz++){
                    bw.write(redescriptions.get(i).targetValueCounts.get(zz)+" ");
             }
             bw.write("\n\n");
    ...
    void writePlots(String output, ApplicationSettings appset, DataSetCreator dat, Mappings map){
        ...
        FileWriter fw = new FileWriter(file.getAbsoluteFile());
          ...
          for(int i=0;i<redescriptions.size();i++){
              ...
              double finRSS=ruleSize/appset.ruleSizeNormalization;
              if(finRSS>1.0)
                  finRSS=1.0;
   ...
   ArrayList<RedescriptionSet> createRedescriptionSetsCoocGenPred(RedescriptionSet rs ,ArrayList<double []> weights, int jsType,ApplicationSettings appset, DataSetCreator dat, Mappings map, CoocurenceMatrix cooc){
       System.out.println("Predictive set construction...");
       ArrayList<RedescriptionSet> sets=new ArrayList<>();
...
//compute from struct
            ...
            double max = 0.0, tmpZ = 0.0;
            for(int zz = 0;zz<rs.redescriptions.get(i).targetValueCounts.size();zz++){
                tmpZ = rs.redescriptions.get(i).targetValueCounts.get(zz)/(double)rs.redescriptions.get(i).elements.size();
                if(tmpZ>max)
                    max = tmpZ;
            }
        ...
        System.out.println("FI: "+firstInd);
        if(firstInd==-1)
            continue ;
        sets.get(it).redescriptions.add(rs.redescriptions.get(firstInd));
        //redescriptions.add(rs.redescriptions.get(firstInd));
